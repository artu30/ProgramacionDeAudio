...
AudioBuffer* AudioBuffer::load(const char* filename) {
	if (filename) {
		...
		if (extension == "wav") {
			...
			stb_vorbis* structure = stb_vorbis_open_filename(filename, nullptr, nullptr);

			stb_vorbis_info oggInfo = stb_vorbis_get_info(structure);

			unsigned int lengthInSamples = stb_vorbis_stream_length_in_samples(structure);

			lengthInSamples *= oggInfo.channels;

			short* buffer = new short[lengthInSamples];

			int dataBlock = stb_vorbis_get_samples_short_interleaved(structure, oggInfo.channels, buffer, lengthInSamples);

			ALuint bufferAL;
			alGenBuffers(1, &bufferAL);
			...
			alBufferData(bufferAL, format, buffer, lengthInSamples, oggInfo.sample_rate);
			
			AudioBuffer *audioBuffer = new AudioBuffer(bufferAL);
